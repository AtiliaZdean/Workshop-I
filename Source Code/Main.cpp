		...
		if (signin == 'C' || signin == 'c')
		{
			...
			while (loop)
			{
				...
				case -1:
					...
					break;
				case 0:
					CustomerRegistration();  //to register new customer
					break;
				case 1:
					CustomerLogin();  //existing customer login
					break;
				case 2:
					loop = false; break;
				default: break;
		...
		else if (signin == 'S' || signin == 's')
		{
			...
			while (loop)
			{
				...
				case -1:
					...
					break;
				case 0:
					StaffRegistration();  //to register new staff
					break;
				case 1:
					StaffLogin();  //existing staff login
					break;
				case 2:
					loop = false; break;
				default: break;
...
void MainPage()  //displaying home page: welcome message, packages available, information about service
{
...
void StaffRegistration()  //registering new staff
{
	S_Menu menu;
...
void StaffLogin()  //existing staff log in
{
	S_Menu menu;
	menu.header = "       Login       ";
	menu.addOption("        Email       ");
	...
	while (1)
	{
		...
		case 1:
			cout << "\n\t\t\t\t\t\t\t\tPassword " << char(175) << " ";
			cin >> SS.S_Password;
			menu.setValue(1, HidePassword(SS.S_Password));
			break;
		case 2:
...
void StaffHome(S_Staff SS)  //page when when staff successfully logged in
{
	S_Menu menu;
...
void CustomerRegistration()  //registering new customer
{
	S_Menu menu;
	menu.header = "    Registration   ";
...
void CustomerLogin()  //existing customer log in
{
	S_Menu menu;
	menu.header = "       Login       ";
	menu.addOption("        Email       ");
	...
	while (1)
	{
		...
		case 1:
			cout << "\n\t\t\t\t\t\t\t\tPassword " << char(175) << " ";
			cin >> SA.C_Password;
			menu.setValue(1, HidePassword(SA.C_Password));
			break;
		case 2:
...
void CustomerHome(S_Account SA)  //page when when customer successfully logged in
{
	S_Menu menu;
...
S_Account CustomerEdit(S_Account SA) // edit customer's account
{
	S_Menu menu;
...
void Feedback(S_Account SA)  // for customers to both see reviews from other customers and leave reviews
{
	S_Reservation SR;
	vector <S_Review> review;
...
void Action(S_Account SA)  //page for customer that want to act regarding reservation request
{
	S_Reservation SR;
	SR.CustomerID = SA.CustomerID;  //setting CustomerID in reservation database to be the same as logged-in customer's CustomerID
...
void ViewReservations(S_Account SA)  //displaying all reservations made by the logged-in customer
{
	...
	while (1)
	{
		...
		switch (option)
		{
			...
			case 3:
				...
				break;
			case 4:
				cout << "\n\t\t\t\t\t\t\t\tTime [HH:MM 24 Hours System] " << char(175) << " ";
				cin >> SR.R_Time;
...
bool CheckDate(const string& dateinput)
{
	...
	//check if user's entered date is exceeding current date & time, and is actually exist. If both satisfied, return true.
